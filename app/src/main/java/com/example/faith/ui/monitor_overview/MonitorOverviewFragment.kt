...
class MonitorOverviewFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding = DataBindingUtil.inflate<MonitorOverviewFragmentBinding>(
            inflater,
            R.layout.monitor_overview_fragment, container,
            false
        )
        ...
        val adapter = MonitorPostAdapter(
            MonitorPostListener() { postId, operation ->
                if (operation == 1) {
        ...
        viewModel.monitorPosts.observe(
            viewLifecycleOwner,
            Observer {
        ...
        viewModel.changePostState.observe(
            viewLifecycleOwner,
            Observer { post ->
                post?.let {
